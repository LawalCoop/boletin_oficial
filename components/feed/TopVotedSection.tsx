...
export function TopVotedSection() {
  const [data, setData] = useState<TopVotedData | null>(null);
  ...
  if (!data || !data.topFavor || !data.topContra || (data.topFavor.length === 0 && data.topContra.length === 0)) {
    return null;
  }
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-text-primary group-hover:text-accent transition-colors line-clamp-2 leading-snug">
                    {titles[article.slug] || article.slug}
                  </p>
